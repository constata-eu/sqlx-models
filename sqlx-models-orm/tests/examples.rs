...
async fn tutorial() -> anyhow::Result<()> {
  ...
  */
  ...
  assert_vec!(app.human().select().likes_dogs_too_eq(false).all().await?, bob, carol, eve);
  ...
  */
  {
    let same_humans = app.human().select().use_struct(SelectHuman{
      limit: Some(2),
  ...
  assert_eq!(app.human().select().likes_dogs_too_eq(true).count().await?, 1);

  /* Filter but fetch a single item, returning error if not found */
  assert_eq!(app.human().select().likes_dogs_too_eq(true).one().await?, alice);

  /* Filter and get a single Optional item, instead of returning error when not found */
  assert_eq!(app.human().select().likes_dogs_too_eq(true).optional().await?.as_ref(), Some(&alice));
  ...
  impl InsertCatHub {
    pub async fn validate_and_save(self) -> anyhow::Result<Cat> {
      if let Some(human_id) = self.human_id() {
        ...
        let owned = self.state.cat().select().human_id_eq(human_id).count().await?;
  ...
  */
  impl From<(&'static str, Personality, Option<i32>)> for InsertCat {
    fn from(v: (&'static str, Personality, Option<i32>) ) -> InsertCat {
      InsertCat{ id: v.0.to_string(), personality: v.1, human_id: v.2 }
    }
  }

  let garfield = app.cat()
  ...
  */
  ...
  let ball = app.toy().insert(InsertToy{
    name: "Ball".to_string(),
    human_owner: alice.attrs.id
  }).save().await?;

  let rope = app.toy().insert(InsertToy{
    name: "Rope".to_string(),
    human_owner: carol.attrs.id
  }).save().await?;

  app.cat_toy().insert(InsertCatToy{
  ...
  */
  {
    ...
    assert_eq!(garfield.human().await?.expect("Garfield to have a human"), alice);
    ...
    for cat_toy in garfield.cat_toy_vec().await? {
      let others = cat_toy
        .toy().await?
        .cat_toy_scope().cat_id_ne(garfield.id()).all().await?;
  ...
  */
  {
    let matches = app.human().people_who_like_dogs_or_whose_name_is(
      "Bob".to_string(), HumanOrderBy::Name, 1, 1
    ).all().await?;
  ...
  */
  ...
  assert_eq!(alice.attrs.is_allowed_unlimited_cats, true); 
  assert_eq!(alice.attrs.likes_dogs_too, true); // This one we didin't try to change.
  ...
  /*
   ...
   them updated.
  */
  let updated_eve_clone = eve.clone().update()
    .name("Eve Evenson".to_string())
    .age(Some(33))
    .save().await?;

  assert_eq!(updated_eve_clone.attrs, HumanAttrs{
    id: 4,
    name: "Eve Evenson".to_string(),
    age: Some(33),
    is_allowed_unlimited_cats: false,
  ...
  */
  ...
  assert_vec!( humans().name_not_like("Eve%").all().await?, alice, bob, carol);
  assert!( humans().name_like("eve%").all().await?.is_empty() );

  assert_vec!( humans().name_ilike("eve%").all().await?, eve);
  assert_vec!( humans().name_not_ilike("eve%").all().await?, alice, bob, carol);

  assert_vec!( humans().name_similar_to("Eve%").all().await?, eve);
  assert_vec!( humans().name_not_similar_to("Eve%").all().await?, alice, bob, carol);
  ...
  assert_eq!(
    app.cat().select().personality_in(&vec![Personality::Playful]).one().await?,
    felix
  );
  ...
  */
  ...
  let insert_susan = serde_json::from_str::<InsertHuman>(r#"{
    "name":"Susan",
    "age":null,
    "likes_dogs_too":"true"
  ...
  */
  ...
  app.alternate_human().insert(InsertAlternateHuman{
    name: "Ned".to_string(),
    likes_dogs_too: true,
